...
class Agent:
    ...
    def __init__(self, obs_dim, act_dim, global_obs_dim, actor_lr, critic_lr):
        self.option_dim = 1
        self.actor = MLPNetwork(obs_dim + self.option_dim, act_dim)
        ...
        self.option = torch.randn(self.option_dim)
    ...
    def action(self, obs, model_out=False):
        ...
        logits = self.actor(torch.cat((obs, self.option)))  # torch.Size([batch_size, action_size])
    ...
    def target_action(self, obs):
        ...
        logits = self.target_actor(torch.cat((obs, self.option)))  # torch.Size([batch_size, action_size])
        # action = self.gumbel_softmax(logits)
        action = F.gumbel_softmax(logits, hard=True)
